...
## names of the measurements - these become the column names for the test results in "X_test" above
features <- "../UCI HAR Dataset/features.txt"

## Read the activities into a data frame and give it columns names
actdf <- read.table(activity_names, header=FALSE, sep=" ", col.names=(c("activity_id", "activity_name")))

## read the ids of the tests
y_testdf <- read.table(y_test, header=FALSE, col.names = c("activity_id"))

## and merge these two data frames based on the commmon id being careful to avoid
## changing the order (sort=FALSE)
mergedf <- merge(y_testdf, actdf, by.x="activity_id", by.y="activity_id", all=TRUE, sort=FALSE)

## now read the subject id (i.e. person id) and give it a column name
subdf <- read.table(subject, header=FALSE, col.names=("subject_id"))
...
testvardf <- read.table(X_test, header=FALSE, strip.white = TRUE)

## and read the column header names
featdf <- read.table(features, header=FALSE, col.names=c("id", "feature"))

## Now remove the open/close parenthesis "()" at the end of most of the 
## feature names to make column headings more human friendly
featdf <- as.data.frame(sapply(featdf, gsub, pattern="\\()", replacement=""))
...
#  clean up memory
rm(list=c("actdf", "y_testdf", "mergedf", "subdf", "fulldf", "testvardf", "featdf", "name"))
...
## names of the sensors - become the column names for the test results in "X_test" above
features <- "../UCI HAR Dataset/features.txt"
...
y_traindf <- read.table(y_train, header=FALSE, col.names = c("activity_id"))

## and merge these two data frames based on the commmon id being careful to avoid
## changing the order (sort=FALSE)
mergedf <- merge(y_traindf, actdf, by.x="activity_id", by.y="activity_id", all=TRUE, sort=FALSE)
...
## Now read the big results table
trainvardf <- read.table(X_train, header=FALSE, strip.white = TRUE)
...
#  clean up memory
rm(list=c("actdf", "y_traindf", "mergedf", "subdf", "fulldf", "trainvardf", "featdf","name"))

## and the individual data frames

rm(list=c("test_finaldf", "train_finaldf"))
...
## for this I'm going to use dplyr
library(dplyr)

## Place the merged data frame into a data frame table suitable for use with dplyr
data <- tbl_df(combineddf)

## and select only the row headers plus any measurement variable containing the words "mean" or "std"
sub_df <- select(data, subject_id, activity_name, contains("mean"), contains("std"))
...
write.table(final_tidy_df, file="final tidy submission.txt", sep=" ", row.names=FALSE)
